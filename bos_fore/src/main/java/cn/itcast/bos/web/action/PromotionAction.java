	...
	public String pageQuery() {
		...
		PageBean<Promotion> pageBean = WebClient
				.create(Constants.BOS_MANAGEMENT_URL
						...
						+ page + "&rows=" + rows)
				.accept(MediaType.APPLICATION_JSON).get(PageBean.class); //  /bos_management
	...
	public String showDetail() throws Exception {
		...
		if (!htmlFile.exists()) {
			...
			Promotion promotion = WebClient
					...
					.get(Promotion.class);  //  /bos_management
...
//		ServletActionContext.getResponse().getWriter().println("<h1>写给浏览器的</h1>");
